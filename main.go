...
func run() error {
	...
	flag.IntVar(&syncInterval, "interval", 30, "Interval on which to poll GitOps repo.")
	...
	// build gitops options based on user configuration.
	syncOpts := []func(*gitops.Sync){}
	syncOpts = append(syncOpts, gitops.WithSSHInfo(syncSSHKeyPath, syncSSHKeyPassword))
	syncOpts = append(syncOpts, gitops.WithRepoInfo(syncRepo, syncBranch))
