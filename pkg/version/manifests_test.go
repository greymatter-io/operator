			...
			tests: []test{
				...
				},
				{
					...
					run: func(t *testing.T, v *Version) {
						...
						if count := len(namespaces); count != 1 {
							t.Fatalf("Expected len(namespaces) to be 1 but got %d: %v", count, namespaces)
						}
						if namespaces[0] != "myns" {
							t.Errorf("expected 'myns' in namespaces, got %s", namespaces[0])
						}
					},
				},
				{
...
func TestBasicMeshWithOptions(t *testing.T) {
	testManifestOutputs(t,
		manifestArgs{
			mesh: &v1alpha1.Mesh{
				ObjectMeta: metav1.ObjectMeta{Name: "default-mesh"},
